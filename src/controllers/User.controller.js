...
const getUserPlaylists = asyncHandler(async (req, res) => {
  ...
  const channel = await User.aggregate([
    ...
    },
    {
      $project: {
        ...
        coverImage: 1,
        email: 1,
        // Moving totalViews calculation to playlists in $project
        playlists: {
          $map: {
            input: "$playlists",
            as: "playlist",
            in: {
              ...
              // Total views for each playlist calculated here
              totalViews: {
                $sum: {
                  $map: {
  ...
  // console.log(channel[0].playlists[0].videosInfo[0])
